...
use crate::ui::{document::document_workspace::Panel, view::{color_palette::{self, ColorPaletteView}, layers::LayersView, toolbar::ToolbarView}};
...
impl<'a> TabViewer for DocumentWorkspacePanelViewer<'a> {
    ...
    fn title(&mut self, tab: &mut Self::Tab) -> egui::WidgetText {
        match tab {
            Panel::Palette => {
                self.color_palette.id().into()
            },
            Panel::Layers => {
                self.layers.id().into()
            },
            Panel::Toolbar => {
                self.toolbar.id().into()
            },
        }
    }
